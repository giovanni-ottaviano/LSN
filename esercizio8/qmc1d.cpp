...
double variationalWaveFunction_second (double v) {
    double mu = 0.81, sigma = 0.6;
    double Zm = (v - mu)/sigma;
    double Zp = (v + mu)/sigma;

    //	return 0.;
    return ( exp(-Zm*Zm/2.)*(1. - Zm*Zm) + exp(-Zp*Zp/2.)*(1. - Zp*Zp) )/(sigma*sigma);
...
void readInput() {
	...
	char* string_away = new char[60];      //SI PUò RIFARE STATIO
